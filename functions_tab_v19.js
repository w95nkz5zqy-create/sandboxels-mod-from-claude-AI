// Functions Tab v19 - Button duration, extension distance, saves with world

console.log('Functions Tab v19 - Starting load...');

setTimeout(function() {
    try {
        console.log('Functions Tab v19 - Initializing...');
        ...
        // Hook into world save/load
        if (typeof saveState !== 'undefined') {
            var originalSaveState = window.saveState || saveState;
            window.saveState = saveState = function() {
                ...
                console.log('Saved functions to world file');
                return state;
            };
        }
        
        if (typeof loadState !== 'undefined') {
            var originalLoadState = window.loadState || loadState;
            window.loadState = loadState = function(state) {
                var result = originalLoadState.apply(this, arguments);
                if (state && state.functionGroups) {
                    try {
                        groups = JSON.parse(state.functionGroups);
                        console.log('Loaded functions from world file:', groups.length, 'groups');
        ...
        function exec(func) {
            ...
            func.pixels.forEach(function(pd) {
                ...
                if (func.type === 'passable') {
                    ...
                    var oldExtensions = func.extensions.slice();
                    func.extensions = [];
                    
                    oldExtensions.forEach(function(e) {
                        if (pixelMap[e.x] && pixelMap[e.x][e.y]) {
                            var elem = pixelMap[e.x][e.x][e.y].element;
                    ...
                    if (func.active && func.value > 0) {
                        ...
                        var dist = func.control === 'slider' ? Math.floor(maxDist * (func.value / 100)) : maxDist;
        ...
        console.log('Functions Tab v19 LOADED!');
        console.log('Saves to world file + button duration + extension distance');
        
    } catch (e) {
        console.error('Functions Tab v19 ERROR:', e);
    }
}, 1000);
