...
(function() {
    ...
    function loadMod() {
        ...
        // Modified Uranium-235 - EXPLOSION RESISTANT, no fission products
        elements.modified_uranium_235 = {
            ...
            hardness: 0.9,
            temp: 20,
            tempHigh: 1500, // Lowered melting point
        ...
        // Barium Absorber - EXPLOSION RESISTANT
        elements.barium_absorber = {
            ...
            tick: function(pixel) {
                for (var dx = -4; dx <= 4; dx++) {
                    for (var dy = -4; dy <= 4; dy++) {
                        if (dx === 0 && dy === 0) continue;
                        var bx = pixel.x + dx, by = pixel.y + dy;
                        if (!isEmpty(bx, by, true) && pixelMap[bx] && pixelMap[bx][by]) {
                            ...
                            if (bPixel && (bPixel.element === "barium_141" || bPixel.element === "barium_141_solid")) {
                                var dirX = pixel.x - bx, dirY = pixel.y - by;
                                if (Math.abs(dirX) <= 1 && Math.abs(dirY) <= 1) {
                                    deletePixel(bx, by);
                                    return;
                                } else {
        ...
        console.log('====================================');
        console.log('FISSION MOD v42 LOADED!');
        console.log('Changes:');
        console.log('- NO barium/xenon production');
        console.log('- Modified U-235 melts at 1500°C');
        console.log('- Neutrons pass through water');
        console.log('- Explosion-resistant absorbers/rods');
        console.log('====================================');
    }
})();
