// Functions Tab v20 - Move back, extensions fixed, saves to world

console.log('Functions Tab v20 - Starting load...');

setTimeout(function() {
    try {
        console.log('Functions Tab v20 - Initializing...');
        ...
        // Hook into Sandboxels save/load BEFORE they're called
        setTimeout(function() {
            console.log('Hooking save/load functions...');
            
            // Hook saveState
            if (typeof window.saveState !== 'undefined') {
                var originalSave = window.saveState;
                window.saveState = function() {
                    console.log('saveState called - saving functions');
                    var state = originalSave.apply(this, arguments);
                    state.functionGroups = JSON.stringify(groups);
                    console.log('Functions saved to world:', groups.length, 'groups');
                    return state;
                };
            }
            
            // Hook loadState
            if (typeof window.loadState !== 'undefined') {
                var originalLoad = window.loadState;
                window.loadState = function(state) {
                    console.log('loadState called - loading functions');
                    var result = originalLoad.apply(this, arguments);
                    if (state && state.functionGroups) {
                        try {
                            groups = JSON.parse(state.functionGroups);
                            console.log('Functions loaded from world:', groups.length, 'groups');
        ...
        try {
            ...
            if (saved) {
                ...
                console.log('Loaded from localStorage:', groups.length, 'groups');
            }
        } catch (e) {}
        ...
        function makeFunc(func, idx, group) {
            ...
            if (func.control === 'toggle') {
                ...
                btn.style.cssText = 'flex: 1; padding: 15px; background: #3498db; color: white; border: none; cursor: pointer; font-weight: bold;';
                
                var durInput = document.createElement('input');
                durInput.type = 'number';
                durInput.value = func.duration || 1;
                durInput.min = '0.1';
                durInput.step = '0.1';
                durInput.style.cssText = 'width: 60px; padding: 5px; background: #34495e; color: white; border: 1px solid black;';
                durInput.onchange = function() {
                    func.duration = parseFloat(durInput.value);
                };
                ...
                btnRow.appendChild(durInput);
                btnRow.appendChild(secLabel);
        ...
        function showFuncForm(group, groupDiv) {
            ...
            typeSelect.innerHTML = '<option value="passable">Passable</option><option value="shock">Shock</option><option value="move_up">Move Up</option><option value="move_down">Move Down</option><option value="move_left">Move Left</option><option value="move_right">Move Right</option><option value="extend_up">Extend Up</option><option value="extend_down">Extend Down</option><option value="extend_left">Extend Left</option><option value="extend_right">Extend Right</option>';
            ...
            // Extension distance input
            var extendDiv = document.createElement('div');
            extendDiv.style.cssText = 'display: none; margin-bottom: 10px;';
            
            var extendLabel = document.createElement('div');
            extendLabel.textContent = 'Max Extension Distance (pixels):';
            extendLabel.style.cssText = 'margin-bottom: 5px; color: #3498db;';
            
            var extendInput = document.createElement('input');
            extendInput.type = 'number';
            extendInput.value = '10';
            extendInput.min = '1';
            extendInput.max = '50';
            extendInput.style.cssText = 'width: 100%; padding: 10px; background: #34495e; color: white; border: none;';
            
            extendDiv.appendChild(extendLabel);
            extendDiv.appendChild(extendInput);
            
            typeSelect.onchange = function() {
                extendDiv.style.display = typeSelect.value.startsWith('extend_') ? 'block' : 'none';
            };
            ...
            createBtn.onclick = function() {
                ...
                group.funcs.push({
                    ...
                    value: 0,
                    duration: 1,
                    extendDistance: parseInt(extendInput.value) || 10,
                    extensions: []
                });
        ...
        function exec(func) {
            ...
            func.pixels.forEach(function(pd) {
                ...
                if (func.type === 'passable') {
                    ...
                    // Shock toggle - continuously charge while active, prevent heating
                    if (func.active) {
                        // Store original temp if not already stored
                        if (!p.shockBaseTemp) {
                            p.shockBaseTemp = p.temp;
                        }
                        p.charge = 100;
                        p.chargeCD = 100;
                        // Force temp back to original every execution
                        p.temp = p.shockBaseTemp;
                    } else {
                        p.charge = 0;
                        p.chargeCD = 0;
                        // Clear stored temp when turned off
                        delete p.shockBaseTemp;
                    }
                    ...
                    console.log('Deleting', func.extensions.length, 'old extensions');
                    for (var i = func.extensions.length - 1; i >= 0; i--) {
                        var ext = func.extensions[i];
                    ...
                    if (func.active && func.value > 0) {
                        var maxDist = func.extendDistance || 10;
                        var dist = Math.floor(maxDist * (func.value / 100));
                        
                        console.log('Creating', dist, 'duplicates of', p.element, 'in direction', dir);
                        
                        // Create duplicates of the original pixel
                        for (var i = 1; i <= dist; i++) {
                            var ex = p.x + (dx * i);
                            var ey = p.y + (dy * i);
                            
                            if (isEmpty(ex, ey)) {
                                // Duplicate the SAME element as the original
                                createPixel(p.element, ex, ey);
                                func.extensions.push({x: ex, y: ey});
                                console.log('Created duplicate at', ex, ey);
                            } else {
                                console.log('Position blocked at', ex, ey);
                            }
                        }
                        
                        console.log('Total duplicates created:', func.extensions.length);
                    } else {
                        console.log('Not extending - active:', func.active, 'value:', func.value);
                    }
                }
            });
        }
        
        console.log('Functions Tab v21 LOADED!');
        console.log('Move back + Extensions duplicate + World save + Continuous shock');
        
        // Continuous execution loop for active toggles
        setInterval(function() {
            groups.forEach(function(group) {
                executeGroupFunctions(group);
            });
        }, 100); // Run every 100ms
        
        function executeGroupFunctions(group) {
            if (group.funcs) {
                group.funcs.forEach(function(func) {
                    if (func.active && func.control === 'toggle') {
                        // Re-execute active toggles continuously
                        exec(func);
                    }
                });
            }
            ...
            if (group.subGroups) {
                group.subGroups.forEach(function(sg) {
                    executeGroupFunctions(sg);
                });
            }
        }
        
    } catch (e) {
        console.error('Functions Tab v20 ERROR:', e);
    }
}, 1000);
